...
def ccista(S, lambda1, epstol=1e-5, maxitr=100, penalize_diagonal=False):
    ...
    hist_delta_subg = np.full((maxitr, 1), -1, order="F", dtype='float64')
    hist_hn = np.full((maxitr, 1), -1, order="F", dtype='float64')

    # return _cc.ccista(S, lambda1, lambda2, epstol, maxitr, steptype)
    Xn = _cce.ccista(S, lambda1, epstol, maxitr, hist_inner_itr_count, hist_delta_subg, hist_hn)

    hist = np.hstack([hist_inner_itr_count, hist_delta_subg, hist_hn])
