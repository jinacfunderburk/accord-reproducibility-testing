...
SparseMatrix<double> ccista_backtracking(
    ...
    while (true) {
        ...
        ////////// For fixed step size simulation ////////////
        // tmp = MatrixXd(X) - tau*grad_h1;
        // sthreshmat(tmp, tau, LambdaMat);
        // Xn = tmp.sparseView();
        // Step = Xn - X;
...
// //////////// check inner loops and if descent condition is satisfied ////////////
...
//     double Q, hn, h1, h1n, successive_norm_updates, norm_updates;
//     double c_ = 0.5;
//     double tau_;
//     double elapsed;
...
//         grad_h1 = Wn + lam2 * Xn;
//         hn = h1n - Xn.diagonal().array().log().sum() + Xn.cwiseAbs().cwiseProduct(LambdaMat).sum();
//         successive_norm_updates = Step.norm();
//         norm_updates = (Xn - Omega_star).norm();

//         hist_inner_itr_count(outer_itr_count) = inner_itr_count;
//         hist_hn(outer_itr_count) = hn;
//         hist_successive_norm(outer_itr_count) = successive_norm_updates;
//         hist_norm(outer_itr_count) = norm_updates;
//         hist_iter_time(outer_itr_count) = elapsed;
        
//         outer_itr_count += 1;

//         if (norm_updates < epstol || outer_itr_count >= maxitr) {
//             if (outer_itr_count <= maxitr) {
//                 hist_inner_itr_count(outer_itr_count) = -1;
//                 hist_hn(outer_itr_count) = -1;
//                 hist_successive_norm(outer_itr_count) = -1;
...
//     return Xn; // return Omega, not Theta
// }